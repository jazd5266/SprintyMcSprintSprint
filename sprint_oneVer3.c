...
int main (int argc, char *argv[])
{
	...
	if ( argc == 3 ) 
	{
		...
		if ( *carr2 == 's' )
		{
			...
			fprintf(fp, "%s", "User: ");
			fwrite ( cusername, 4, sizeof (char), fp );
			
			fputs ("_", fp);

			fprintf(fp, "%d", ifinalnum);

			fprintf(fp, "%s", "   Your score is: ");
			fprintf(fp, "%d", iscore);			
...
int codenamescorer(char *cusername)
{
	...
	//Assign ordinal value to an int. For loop, to loop through string for strlen (length of string). Add each letter's ordinal value to an int
